...
class PlayState extends MusicBeatState
{
	...
	public static var blackShit:FlxSprite;
	public static var epicText:FlxText;

	public static var rep:Replay;
	public static var loadRep:Bool = false;
	public static var shitThing:Bool = false;
	public static var coolThing:Bool = false;

	public static var noteBools:Array<Bool> = [false, false, false, false];

	var epicdoof:DialogueBox;
	...
	public var epic:Array<String> = ['dad:blah blah blah', 'bf:coolswag'];

	var epicBoppers:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'picnic':
				dialogue = CoolUtil.coolTextFile(Paths.txt('picnic/teddy'));
			case 'fun':
				dialogue = CoolUtil.coolTextFile(Paths.txt('fun/funni'));
			case 'end-of-episode':
				// spong best mod
				dialogue = CoolUtil.coolTextFile(Paths.txt('end-of-episode/hehehguheh'));
				epic = CoolUtil.coolTextFile(Paths.txt('end-of-episode/noooomoooorebean'));
		...
		switch(SONG.stage)
		{
			...
			case 'beanstage':
			{
					...
					{
						shiny = new FlxSprite(0, -50).loadGraphic(Paths.image('shinyBlock'));
						shiny.scrollFactor.set(0.9, 0.9);
						shiny.screenCenter(Y);
						shiny.screenCenter(X);
						shiny.visible = false;
					}
					else
					{
						shiny = new FlxSprite(0, 50).loadGraphic(Paths.image('shinyBlockUP'));
			            shiny.scrollFactor.set(0.9, 0.9);
					    shiny.screenCenter(X);
					    shiny.visible = false;
					}
			    switch (SONG.song.toLowerCase())
			    {
				    ...
				    {
				        curStage = 'beanstage';
					    var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('beanstage'));
					    bg.antialiasing = true;
				    ...
				    {
					    curStage = 'beanstage';
					    var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('beannight'));
					    bg.antialiasing = true;
					    bg.scrollFactor.set(0.9, 0.9);
					    bg.active = false;
					    add(bg);
				...
				epicBoppers = new FlxSprite(0, 220);
				epicBoppers.frames = Paths.getSparrowAtlas('bop','shared');
				epicBoppers.animation.addByPrefix('bop', 'Bop', 24, false);
				epicBoppers.scrollFactor.set(0.9, 0.9);
				epicBoppers.screenCenter(X);
				epicBoppers.updateHitbox();
				epicBoppers.antialiasing = true;
				add(epicBoppers);
		...
		epicdoof = new DialogueBox(false, epic);
		epicdoof.scrollFactor.set();
		epicdoof.finishThing = endSong;
		...
		healthBar.createFilledBar(0xFF8B4513, 0xFF00FFFF);
		// healthBar
		add(healthBar);

		// Add Kade Engine watermark
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "VERY Hard" : storyDifficulty == 1 ? "Bean" : "Easy") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (songLowercase == 'end-of-episode')
					{
						FlxG.save.data.hardModeUnlocked = true;
						FlxG.save.data.Completed = true;
						trace("hardMode can be accessed:" + FlxG.save.data.Completed + ".");
						if (FlxG.save.data.Completed = null)
						{
							trace("BRO WTF WHY AM I NULL???");
						}
						else if (FlxG.save.data.Completed = true)
						{
							trace("lucky you bean boy");
						}
					}
					...
					if (songLowercase == 'end-of-episode')
					{
						...
						epicText = new FlxText(-100, 0, 0, "VERY VERY VERY BIG THANKS TO:", 64);
						epicText.setFormat(Paths.font("vcr.ttf"), 64);
						epicText.screenCenter(Y);
						add(epicText);
					...
					FlxG.save.flush();
					// LOL this is actualyy just useless but eh
	...
	*/
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'beanstage':
				epicBoppers.animation.play('bop', true);
			case 'limo':
				if(FlxG.save.data.distractions){
